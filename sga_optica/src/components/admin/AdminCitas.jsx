...
import { Plus, Pencil, Trash2, X, Check, Search, RefreshCw, Ban } from 'lucide-react'
...
const EMPTY = {
  customerId: '', optometristId: '', examTypeId: '',
  appointmentDate: '', startTime: '', notes: '', status: 'PENDING'
}
...
export default function AdminCitas() {
  ...
  const load = async () => {
    ...
    try {
      const [aRes, cRes, oRes, eRes] = await Promise.all([
        getAppointments(), getCustomers(), getOptometrists(), getExamTypes()
      ])
  ...
  const openEdit = (a) => {
    setForm({
      ...
      optometristId:   a.optometristId  || '',
      examTypeId:      a.examTypeId     || '',
      appointmentDate: a.appointmentDate ? a.appointmentDate.split('T')[0] : '',
  ...
  const handleSave = async (e) => {
    ...
    try {
      const payload = {
        ...
        optometristId: parseInt(form.optometristId),
  ...
  const handleCancel = async (id) => {
    ...
    try {
      await cancelAppointment(id); setSuccess('Cita cancelada'); load()
      setTimeout(() => setSuccess(''), 4000)
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteAppointment(id); setSuccess('Cita eliminada'); load()
      setTimeout(() => setSuccess(''), 4000)
    } catch (e) { setError(e.response?.data?.message || 'Error') }
  }
  ...
  const optoName = (id) => {
    ...
    return o ? `${o.firstName} ${o.firstLastName}` : `#${id}`
  }
  const examName = (id) => {
    const e = examTypes.find(e => e.id_exam_type === id)
      ...
      {loading ? <div className="loading-spinner"><div className="spinner" /></div> : (
        <div className="table-wrapper">
          <table className="admin-table">
            <thead>
              <tr><th>Cliente</th><th>Optómetra</th><th>Examen</th><th>Fecha</th><th>Hora</th><th>Estado</th><th>Acciones</th></tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} className="empty-row">No hay citas</td></tr>
              ) : filtered.map(a => (
                <tr key={a.appointment_id}>
                  ...
                  <td className="actions-cell">
                    ...
                    {a.status !== 'CANCELLED' && a.status !== 'COMPLETED' && (
                      <button className="btn-icon warn" onClick={() => handleCancel(a.appointment_id)} title="Cancelar"><Ban size={15} /></button>
                    )}
              ...
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <select required value={form.examTypeId} onChange={e => setForm({...form, examTypeId: e.target.value})}>
                    ...
                    {examTypes.map(ex => <option key={ex.id_exam_type} value={ex.id_exam_type}>{ex.exam_name}</option>)}
                  </select>
                </div>
                <div className="form-group">
                  <label>Estado</label>
                ...
                <div className="form-group">
                  <label>Fecha *</label>
                  <input required type="date" value={form.appointmentDate} onChange={e => setForm({...form, appointmentDate: e.target.value})} />
                </div>
                <div className="form-group">
                  <label>Hora *</label>
                ...
                <div className="form-group form-full">
                  <label>Notas</label>
                  <textarea rows={3} value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} placeholder="Observaciones o indicaciones..." />
                </div>
              </div>
              <div className="modal-footer">
                <button type="button" className="btn-secondary" onClick={() => setShowModal(false)}>Cancelar</button>
                <button type="submit" className="btn-primary" disabled={saving}>{saving ? 'Guardando...' : editId ? 'Actualizar Cita' : 'Crear Cita'}</button>
              </div>
            </form>
          </div>
        </div>

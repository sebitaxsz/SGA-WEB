...
import { getNotifications, createNotification, updateNotification, deleteNotification, permDeleteNotif, getCustomers } from '../../services/admin.service'
...
export default function AdminNotificaciones() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [nRes, cRes] = await Promise.all([getNotifications(), getCustomers()])
      setItems(nRes.data?.data || nRes.data || [])
  ...
  const openEdit = (n) => {
    setForm({ customerId: n.customerId || '', subject: n.subject || '', message: n.message || '', type: n.type || 'GENERAL', scheduledFor: n.scheduledFor ? n.scheduledFor.slice(0, 16) : '' })
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editId) { await updateNotification(editId, payload); setSuccess('Notificación actualizada') }
      else { await createNotification(payload); setSuccess('Notificación creada') }
  ...
  const handleSoftDelete = async (id) => {
    ...
    try { await deleteNotification(id); setSuccess('Notificación archivada'); load(); setTimeout(() => setSuccess(''), 3000) }
  ...
  const handlePermDelete = async (id) => {
    ...
    try { await permDeleteNotif(id); setSuccess('Notificación eliminada'); load(); setTimeout(() => setSuccess(''), 3000) }
    catch (e) { setError(e.response?.data?.message || 'Error') }
  }
    ...
    <div className="admin-section">
      <div className="section-header">
        <div>
          ...
          <p className="section-subtitle">{items.length} notificaciones</p>
        </div>
      ...
      {loading ? <div className="loading-spinner"><div className="spinner" /></div> : (
        <div className="table-wrapper">
          <table className="admin-table">
            <thead><tr><th>Destinatario</th><th>Asunto</th><th>Tipo</th><th>Estado</th><th>Enviada</th><th>Acciones</th></tr></thead>
            <tbody>
              {items.length === 0 ? <tr><td colSpan={6} className="empty-row">No hay notificaciones</td></tr>
                : items.map(n => (
                  <tr key={n.notification_id}>
                    <td>{customerName(n.customerId)}</td>
                    <td><strong>{n.subject}</strong><br /><span className="text-sm text-muted">{n.message?.slice(0, 50)}{n.message?.length > 50 ? '…' : ''}</span></td>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-card modal-large" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3>{editId ? 'Editar Notificación' : 'Nueva Notificación'}</h3>
            ...
            <form onSubmit={handleSave} className="modal-form">
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn-primary" disabled={saving}><Send size={15} />{saving ? 'Enviando...' : editId ? 'Actualizar' : 'Crear Notificación'}</button>
              </div>
            </form>
          </div>
        </div>

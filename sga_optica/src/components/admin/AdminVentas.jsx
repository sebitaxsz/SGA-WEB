...
import {
  getSales, getSaleById, deleteSale, createSale,
  getCustomers, getPaymentTypes, getProducts
} from '../../services/admin.service'
import { Trash2, X, Check, Search, RefreshCw, Eye, Plus } from 'lucide-react'
...
/*
  ...
  GET /sales → devuelve ventas con SaleProducts incluidos
*/
...
export default function AdminVentas() {
  ...
  const [paymentTypes, setPayTypes] = useState([])
  ...
  const load = async () => {
    ...
    try {
      ...
      setPayTypes(pRes.data || [])
      setProducts(prRes.data?.data || [])
    } catch (e) {
      setError('Error cargando ventas: ' + (e.response?.data?.message || e.message))
  ...
  const handleSave = async (e) => {
    ...
    try {
      const saleProducts = saleItems
        ...
        .map(i => ({ productId: parseInt(i.productId), quantity: parseInt(i.quantity) }))
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteSale(id); setSuccess('Venta eliminada'); load()
      setTimeout(() => setSuccess(''), 4000)
  ...
  const customerName = (id) => {
    ...
    return c ? `${c.firstName} ${c.firstLastName}` : `#${id}`
  }
  const payName = (id) => {
    const p = paymentTypes.find(p => p.payment_type_id === id)
    return p ? p.payment_type_name : `#${id}`
  }
  const fmt   = (n) => n !== undefined && n !== null ? `$${Number(n).toLocaleString('es-CO')}` : '—'
  const fmtDt = (d) => d ? new Date(d).toLocaleDateString('es-CO', { day:'2-digit', month:'short', year:'numeric' }) : '—'

  const addItem    = ()      => setSaleItems(prev => [...prev, { productId: '', quantity: 1 }])
  const removeItem = (i)     => setSaleItems(prev => prev.filter((_, idx) => idx !== i))
  const updateItem = (i,k,v) => setSaleItems(prev => prev.map((it, idx) => idx === i ? {...it, [k]: v} : it))
    ...
    <div className="admin-section">
      <div className="section-header">
        ...
        <div className="section-actions">
          ...
          <button className="btn-primary" onClick={() => { setForm(EMPTY_SALE); setSaleItems([{ productId:'', quantity:1 }]); setError(''); setShowModal(true) }}>
      ...
      {loading ? <div className="loading-spinner"><div className="spinner" /></div> : (
        <div className="table-wrapper">
          <table className="admin-table">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="empty-row">No hay ventas registradas</td></tr>
              ) : filtered.map(s => (
                <tr key={s.sale_id}>
                  ...
                  <td>{fmtDt(s.saleDate || s.createdAt)}</td>
              ...
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <select required value={form.paymentTypeId} onChange={e => setForm({...form, paymentTypeId: e.target.value})}>
                    ...
                    {paymentTypes.map(p => (
                      <option key={p.payment_type_id} value={p.payment_type_id}>{p.payment_type_name}</option>
                    ))}
                  </select>
                </div>
              </div>
              ...
              <div className="sale-items-section">
                ...
                {saleItems.map((item, i) => (
                  <div key={i} className="sale-item-row">
                    ...
                    >
                      ...
                      {products.map(p => (
                        <option key={p.id} value={p.id}>
                          {p.nameProduct} — {fmt(p.unitPrice)} (stock: {p.stock})
                        </option>
                      ))}
                    </select>
                    <input
                    ...
                    />
                    {saleItems.length > 1 && (
                      <button type="button" className="btn-icon delete" onClick={() => removeItem(i)}><X size={14} /></button>
                    )}
                  </div>
                ))}
              </div>
        ...
        <div className="modal-overlay" onClick={() => setShowDetail(null)}>
          <div className="modal-card" onClick={e => e.stopPropagation()}>
            ...
            <div className="sale-detail">
              ...
              <div className="detail-row"><span>Tipo de pago:</span><strong>{payName(showDetail.paymentTypeId)}</strong></div>
              <div className="detail-row"><span>Fecha:</span><strong>{fmtDt(showDetail.saleDate || showDetail.createdAt)}</strong></div>
              <div className="detail-row"><span>Total:</span><strong className="money">{fmt(showDetail.totalAmount)}</strong></div>
                    ...
                    <tbody>
                      {showDetail.SaleProducts.map((sp, i) => (
                        <tr key={i}>
                          ...
                          <td>{sp.quantity}</td>
                          <td>{fmt(sp.unitPrice)}</td>
                          <td>{fmt(sp.subtotal)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
